 ...
 */
public class FullInfoSplitReaderBeta<S extends Writable, V extends Writable, E extends Writable, K extends Writable, M extends Writable>
        implements
    ...
    public FullInfoSplitReaderBeta(
...
//
    ...
    private void createVertex(String stringInput) {
        ...
        for (int j = 3; j < vertexValue.length; j++) {
            ...
            Edge<E, LongWritable, LongWritable> e = new BiEdge<E, LongWritable, LongWritable>(vertexID,edgeID, sinkID);
            _adjList.add(e);
            if (sinkPartitionID != peer.getPeerIndex()) {
                ...
                controlInfo.addextraInfo(Longs.toByteArray(Long.parseLong(vertexValue[j+1])));
                controlInfo.addextraInfo(Longs.toByteArray(Long.parseLong(vertexValue[j])));
                controlInfo.addextraInfo(Longs.toByteArray(tempEdgeId));
                controlInfo.addextraInfo(Longs.toByteArray(tempVertexId));
                Long pid = Long.parseLong(vertexValue[j+2]);
                try {
                    peer.send(peer.getPeerName(pid.intValue()),inEdgesMessage);} catch (IOException e1) {e1.printStackTrace();
                }
    ...
    private IVertex<V, E, LongWritable, LongWritable> createVertexInstance(LongWritable vertexID, List<IEdge<E, LongWritable, LongWritable>> adjList) {
        return ReflectionUtils.newInstance(GraphJobRunner.BIVERTEX_CLASS, new Class<?>[] {Writable.class, Iterable.class},
